...
(function() {
  ...
  App.Routers.Shop = (function(_super) {
    __extends(Shop, _super);
    ...
    Shop.before({
      these: ['index', 'showProducts', 'showUser', 'showOrder', 'showPaymentMethods'],
      "do": 'setup'
    });

    Shop.prototype.routes = {
      ':id(/)': 'index',
      ':id/products(/)': 'showProducts',
      ':id/user(/)': 'showUser',
      ':id/order(/)': 'showOrder',
      ':id/pay(/)': 'showPaymentMethods',
      ':id/initiate-payment(/)': 'sendToPayment',
      'next(/)': 'showNext',
      'previous(/)': 'showPrevious',
      ':id/u/:uuid(/)': 'uuidLogin'
    };

    Shop.prototype.setup = function(id) {
      if (this.initialized) {
        if (id === this.id) {
          return;
        }
      ...
      App.frontend.shop = App.Models.Shop.get(id);
      App.frontend.order = new App.Models.Order({
        user: App.session.get('user'),
        shop: App.frontend.shop
      });
      this.indexView = new App.Views.Shop.Index({
        shop: App.frontend.shop
      });
      return App.frontend.shop.fetch().then((function(_this) {
        return function() {
          App.frontend.order.save();
          App.window.setView(_this.indexView);
          _this.id = id;
          return _this.initialized = true;
        };
      })(this)).fail((function(_this) {
        return function(rejection) {
          var authenticationView, method;
          if (rejection.response.status === 401) {
            method = rejection.response.getResponseHeader("X-Authenticate");
            authenticationView = (function() {
              switch (method) {
                case 'token':
                  return new App.Views.Shop.RequireToken({
                    shop: App.frontend.shop
                  });
                case 'password':
                  return new App.Views.Shop.RequirePassword({
                    shop: App.frontend.shop
                  });
              }
            })();
            return App.window.setView(authenticationView);
          }
        };
      })(this));
    };

    Shop.prototype.index = function(id) {
      return App.frontend.shop.fetch().then((function(_this) {
        return function() {
          return _this.navigate("" + id + "/products");
        };
      })(this));
    };

    Shop.prototype.showProducts = function(id) {
      App.frontend.shop.fetch().then((function(_this) {
        return function() {
          App.frontend.shop.get('shop_products').fetch();
          return App.frontend.shop.get('categories').fetch();
        };
      })(this));
      this.indexView.setView(this.productsView || (this.productsView = new App.Views.Shop.ShowProducts({
        shop: App.frontend.shop,
        order: App.frontend.order
      })));
      return this.next = function() {
        return this.navigate("" + id + "/user");
      };
    };

    Shop.prototype.showUser = function(id) {
      this.indexView.setView(this.userView || (this.userView = new App.Views.Shop.ShowUser({
        shop: App.frontend.shop,
        user: App.session.get('user')
      })));
      this.previous = function() {
        return this.navigate("" + id + "/products");
      };
      return this.next = function() {
        return this.navigate("" + id + "/order");
      };
    };

    Shop.prototype.showOrder = function(id) {
      this.indexView.setView(this.orderView || (this.orderView = new App.Views.Shop.ShowOrder({
        shop: App.frontend.shop,
        order: App.frontend.order
      })));
      this.previous = function() {
      ...
      return this.next = function() {
        return this.navigate("" + id + "/pay");
      };
    };

    Shop.prototype.showPaymentMethods = function(id) {
      App.frontend.shop.fetch().then((function(_this) {
        return function() {
          return App.frontend.shop.get('shop_payment_methods').fetch();
        };
      })(this));
      this.indexView.setView(this.paymentView || (this.paymentView = new App.Views.Shop.ShowPaymentMethods({
        shop: App.frontend.shop,
        order: App.frontend.order
      })));
    ...
    Shop.prototype.uuidLogin = function(id, uuid) {
      var session;
      session = new App.Models.Session({
        uuid: uuid
      });
      return session.save().then((function(_this) {
        return function() {
          App.setSession(session);
          return _this.navigate("" + id + "/");
        };
      })(this));
    };

    Shop.prototype.reset = function() {
      App.frontend.shop = App.frontend.order = this.payment_method = null;
      return this.indexView = this.productsView = this.userView = this.orderView = this.paymentView = null;
    };
...
(function() {
  App.Frontends.Shop = {
    Router: App.Routers.Shop,
    initialize: function() {
      var deferred, promise, session;
      deferred = Q.defer();
      session = new App.Models.Session;
      promise = session.save();
      promise.then(function() {
...
App.initialize(App.Frontends.Shop);
