...
(function() {
  ...
  App.Routers.Personal = (function(_super) {
    ...
    Personal.before({
      these: ['index', 'downloads', 'user', 'shops', 'support', 'orders', 'order'],
      "do": 'setup'
    });

    Personal.prototype.routes = {
      '(/)': 'user',
      'user(/)': 'user',
      'downloads(/)': 'downloads',
      'shops(/)': 'shops',
    ...
    Personal.prototype.setup = function() {
      var loginView;
      if (this.initialized) {
        return;
      }
      if (App.session && App.session.has('user')) {
        this.indexView = new App.Views.Personal.Index({
    ...
    Personal.prototype.user = function(id) {
      if (!this.initialized) {
        return;
      }
      return this.indexView.setView(this.overviewView || (this.overviewView = new App.Views.Personal.ShowUser({
        user: App.session.get('user'),
        paymentStatus: this.paymentStatus
      })));
    };

    Personal.prototype.downloads = function() {
      App.session.get('user').get('order_product_identifiers').fetch({
        force: true
      });
      return this.indexView.setView(this.orderProductIdentifiersView || (this.orderProductIdentifiersView = new App.Views.Personal.ShowDownloads({
        order_product_identifiers: App.session.get('user').get('order_product_identifiers')
      })));
    };

    Personal.prototype.shops = function() {
      return this.indexView.setView(this.shopsView || (this.shopsView = new App.Views.Personal.ShowShops({
        user: App.session.get('user')
      })));
    };

    Personal.prototype.support = function() {
      return this.indexView.setView(this.supportView || (this.supportView = new App.Views.Personal.ShowSupport()));
    };

    Personal.prototype.orders = function() {
      App.session.get('user').fetch().then(function() {
        return App.session.get('user').get('orders').fetch({
          force: true
        });
      });
      return this.indexView.setView(this.ordersView || (this.ordersView = new App.Views.Personal.ShowOrders({
        orders: App.session.get('user').get('orders')
      })));
    };

    Personal.prototype.orderDetails = function(id) {
      App.Models.Order.get(id).fetch().then(function() {
        return App.Models.Order.get(id).get('payments').each(function(payment) {
          return payment.fetch();
        });
      });
      return this.indexView.setView(this.orderView || (this.orderView = new App.Views.Personal.ShowOrder({
        order: App.Models.Order.get(id)
      })));
    };

    Personal.prototype.uuidLogin = function(uuid) {
      var session;
      session = new App.Models.Session({
        uuid: uuid
      });
      return session.save().then((function(_this) {
        return function() {
          App.setSession(session);
          _this.paymentStatus = _this.paymentStatus();
          return _this.navigate('/');
        };
      })(this));
    };

    Personal.prototype.reset = function() {};

    Personal.prototype.paymentStatus = function() {
...
(function() {
  App.Frontends.Personal = {
    Router: App.Routers.Personal,
    initialize: function() {
      var deferred;
      deferred = Q.defer();
      deferred.resolve();
...
App.initialize(App.Frontends.Personal)
